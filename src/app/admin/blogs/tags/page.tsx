...
import TagsClient, { NewTagButton } from './TagsClient';
...
export default async function BlogTagsPage() {
  ...
  // Count posts per tag (tags stored as text in blogs.tags)
  ...
  (blogs ?? []).forEach((b) => {
    if (b.tags) {
      const tagList = b.tags.split(',').map((t: string) => t.trim()).filter(Boolean);
      tagList.forEach((tag: string) => {
        postCounts[tag] = (postCounts[tag] ?? 0) + 1;
      });
    }
  });
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
        <div className="flex items-start gap-4 mb-5">
          <div className="w-10 h-10 bg-green-50 rounded-xl flex items-center justify-center flex-shrink-0">
            <Tag className="w-5 h-5 text-[#006837]" />
          </div>
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Manage tags to help organize and discover blog posts</p>
          </div>
        </div>
