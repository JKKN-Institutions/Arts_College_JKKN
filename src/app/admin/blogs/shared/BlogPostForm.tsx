...
export default function BlogPostForm({
  ...
  const [postStatus, setPostStatus] = useState(
    initialData?.post_status ?? (initialData?.is_published ? 'published' : 'draft')
  );
  const [readTimeMinutes, setReadTimeMinutes] = useState(
    initialData ? parseReadTimeMinutes(initialData.read_time) : 5
  );
  ...
  async function uploadCoverImage(): Promise<string> {
    if (!coverImageFile) {
      // Edit mode: keep the existing URL; Create mode: return empty string
      return isEdit ? coverImagePreview : '';
    }
  ...
  async function handleSubmit(publish: boolean) {
    ...
    if (isEdit && initialData) {
      ...
      const payload: Record<string, unknown> = {
        ...
        ...(status === 'published' && !initialData.is_published ? { published_at: new Date().toISOString() } : {}),
      };
        ...
        <div className="flex items-start gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {isEdit ? 'Update your blog post content and settings' : 'Create a new blog post with rich content'}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            {coverImagePreview ? (
              <div className="relative">
                {/* Using <img> intentionally: coverImagePreview can be a blob: URL (local file preview) which next/image does not support */}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="flex items-center gap-3">
              {userAvatar ? (
                <Image src={userAvatar} alt={userName} width={40} height={40} className="rounded-full object-cover flex-shrink-0" />
              ) : (
